...
export default function PregnancyStoryPage() {
    ...
    useEffect(() => {
        const fetchPregnancyStory = async () => {
            try {
                const query = `*[_type == "pergnancy_stories" && slug.current == $slug][0]{story ,images[] { asset->{url} } }`;
                const result = await client.fetch(query, { slug });

                if (result) {
                    const pregnancyImages = result.images.map(
                        (item: { asset: { url: string } }) => item.asset.url
                    );
                    setImages(pregnancyImages);
                }
            } catch (error) {
                console.error("Error fetching pregnancy story:", error);
            }
        };
